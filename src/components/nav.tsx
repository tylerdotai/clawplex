...
import { nav as copy } from "@/lib/dict";
    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8 md:py-5">
            ...
            >
              ...
              >
                ...
                <svg
                  ...
                  className={`text-claw-dim transition-transform ${communityHover ? "rotate-180" : ""}`}
                  aria-hidden="true"
                >
          ...
          <button
            ...
            aria-expanded={open}
          >
            <motion.span animate={{ rotate: open ? 45 : 0, y: open ? 6 : 0 }} transition={{ duration: 0.2 }} className="block h-[1.5px] w-5 bg-claw-text origin-center" />
            <motion.span animate={{ opacity: open ? 0 : 1 }} transition={{ duration: 0.15 }} className="block h-[1.5px] w-5 bg-claw-text" />
            <motion.span animate={{ rotate: open ? -45 : 0, y: open ? -6 : 0 }} transition={{ duration: 0.2 }} className="block h-[1.5px] w-5 bg-claw-text origin-center" />
          </button>
        </div>
      </nav>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="flex-1 flex flex-col justify-center px-8 pt-20" onClick={(e) => e.stopPropagation()}>
              ...
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 12 }}
                transition={{ duration: 0.3, delay: (copy.communityLinks.length + copy.links.length) * 0.04 + 0.05 }}
                className="mt-10"
              >
                <a
                  href={primaryCtaHref}
